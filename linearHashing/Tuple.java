...
public class Tuple {
	...
	/**
	 * In this constructor, we define that length is given by len and keys are
	 * an integer array with the same length In keys: key byte are given by
	 * their index against an tuple and the rest are -1. From 0 to len-1, the
	 * importance goes down for example: a 8 bytes long tuple, key is the first
	 * integer (0-3 inclusive) so the keys should be [0,1,2,3,-1,-1,-1,-1]
	 * 
	...
	public Tuple(String fileNameWithPath) {
		try {
			...
			for (int ii = 0; ii < list.getLength(); ii++) {
				...
				if (element.hasAttribute("order") && element.getAttribute("order").equals("descending")) {
					attribute.setDescending(true);
				}
				res.add(attribute);
			}
			...
			this.descending = new boolean[this.length];
			for (int jj = 0; jj < this.length; jj++) {
				this.keyBytes[jj] = -1;
				this.descending[jj] = false;
			}

			int descendingOffset = 0;
			for (int ii = 0; ii < res.size(); ii++) {
				if (res.get(ii).getDescending()) {
					for (int kk = 0; kk < res.get(ii).getLength(); kk++) {
						this.descending[kk + descendingOffset] = true;
					}
				}
				descendingOffset += res.get(ii).getLength();
			}
	...
	/**
	 * if given bytes error, we return 0 -1: one < two, 0: one == two, 1: one >
	 * two
	 * 
	 * @param one
	 * @param two
	...
	public int compare(byte[] one, byte[] two) {
		...
		if (one.length != this.length || two.length != this.length) {
			...
			System.out
					.println("given bytes error: one and two should be the same " + "length as defined in Tuple class");
...
//	}
	...
	public int compareTwoKeys(byte[] one, byte[] two) {
		if (one.length != this.keyLength || two.length != this.keyLength) {

			System.out.println(
					"given bytes error: one and two should be the same " + "keyLength as defined in Tuple class");
		...
		for (int ii = 0; ii < this.keyLength; ii++) {
			...
			if ((one[ii] & 0x0FF) < (two[ii] & 0x0FF)) {
				if (this.descending[pos]) {
					return 1;
				} else {
					return -1;
				}
			} else if ((one[ii] & 0x0FF) > (two[ii] & 0x0FF)) {
				if (this.descending[pos]) {
					return -1;
				} else {
					return 1;
